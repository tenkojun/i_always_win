...
if exist "dist\Plutus\.data" (
  echo   - dist 안의 .data 제거 ^(API 키/인증 DB 포함^)
  rmdir /s /q "dist\Plutus\.data"
)

echo.
echo 완료!
echo   결과물 : dist\Plutus\Plutus.exe
echo   로그   : 실행 후 문제가 있으면 .data\logs\app.log 를 확인하세요.
echo.
echo 더블클릭하면 콘솔 창 없이 앱 창만 열립니다.
echo.
echo [주의] 이 폴더를 남에게 전달하기 전에 Plutus\.data 폴더가
