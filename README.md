...
로그인·가입·승인은 **중앙 인증 서버**(Cloudflare Workers + D1)가 처리한다.
앱에 기본 서버가 내장돼 있어 설치 직후 바로 로그인된다. 자기 서버를
쓰려면 설정 → 중앙 인증에서 주소를 덮어쓰면 된다.

이 PC 에는 계정이 없다. 대신 **브라우저마다 별도 세션 쿠키**를 발급한다.
이 구분이 중요한 이유는 외부 접근 때문이다 — "이 PC 에 로그인된 사람"을
곧 요청자로 삼으면 터널 주소를 아는 사람이 전부 주인 계정으로 들어온다.
그래서 *누구인가*는 중앙 서버가, *이 브라우저가 로그인했는가*는 쿠키가
판단한다.

중앙 서버에 닿지 않을 때(와이파이 끊김 등)는 마지막으로 확인된 신원을
24시간까지 인정한다. 반면 서버가 **명시적으로 거부**하면 즉시 끊는다.
분석 한 번 돌리는 중에 네트워크가 끊겼다고 앱이 잠기면 곤란하기 때문이다.
...
├── auth.db        분석 이력·커뮤니티·보유종목 (계정 아님)
├── browser_sessions.json  브라우저별 세션 ↔ 중앙 토큰
├── session.json   이 PC 의 중앙 세션 (외부 접근 등록용)
├── pc_id          이 PC 식별자
├── chats/         에이전트 대화
├── cache/         가격 캐시
