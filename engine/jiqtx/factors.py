...
# 프로세스 노이즈 q 가 반응 속도를 정한다. 작을수록 매끄럽지만 느리다.
#
# 기본값이 1e-5 였는데 **너무 뻣뻣했다.** 베타가 0.5 → 1.6 으로 계단
# 변화하는 합성 계열에서 반감기가 246일이고, 750일(3년)이 지나도 1.44 에
# 머물렀다(오차 0.156). 같은 함수가 함께 내는 **롤링 252일 창은 1.62 로
# 정확했다** — 개선하려고 넣은 칼만이 단순 롤링보다 못한 상태였다.
# beta_now 는 헤지 사이징에 쓰이므로 10% 과소평가는 그대로 미달헤지다.
#
# 반응성과 매끄러움을 함께 재서 고른 값 (베타 고정 계열의 잡음 σ 도 함께)
#
#     q       반감기   최종β(진짜1.60)   잡음
...
#
# 5e-5 는 추적 오차를 78배 줄이면서(0.156→0.002) 잡음은 1.4%p 만 늘고,
# 반감기가 롤링 252일 창의 구조적 반감기(~126일)와 맞는다.
_KALMAN_Q = 5e-5


def kalman_beta(y: np.ndarray, x: np.ndarray, q: float = _KALMAN_Q,
