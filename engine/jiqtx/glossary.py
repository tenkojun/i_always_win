...
TERMS: Dict[str, Tuple[str, str]] = {
    ...
    # ── v2.21.0 추가 — 보고서 본문에 나오는데 설명이 없던 용어 ──
    "샤프": (
        "샤프 비율 (Sharpe Ratio)",
        "위험 한 단위당 초과수익. 분모가 표준편차라 상승 변동성까지 벌점으로 친다. 표본이 짧으면 쉽게 "
        "부풀려지므로 반드시 PSR·DSR 과 함께 봐야 한다. 샤프 1.0을 t>3 으로 입증하려면 약 9년치 표본이 "
        "필요하다."),
    "소르티노": (
        "소르티노 비율 (Sortino Ratio)",
        "샤프의 분모를 하방 변동성만으로 바꾼 것. 위로 튀는 것은 위험이 아니라는 관점이다. 소르티노가 샤프보다 "
        "훨씬 크면 수익 분포가 우측으로 치우쳐 있다는 뜻 — 큰 손실은 드물지만 한 번에 온다."),
    "칼마": (
        "칼마 비율 (Calmar Ratio)",
        "연수익 ÷ 최대낙폭. 변동성이 아니라 실제로 견뎌야 했던 손실로 나눈다. 운용자가 중도 이탈하는 이유는 "
        "변동성이 아니라 낙폭이라 실무에서 샤프보다 체감에 가깝다. 1 미만이면 견딘 고통만큼 못 벌었다."),
    "왜도": (
        "왜도 (Skewness)",
        "분포의 좌우 비대칭. 음수면 작게 자주 벌고 크게 한 번 잃는 모양 — 옵션 매도·캐리 전략의 전형이다. "
        "이런 분포에서는 평균과 샤프가 위험을 체계적으로 과소평가한다."),
    "첨도": (
        "첨도 (Kurtosis)",
        "꼬리의 두께. 정규분포는 3이다. 그보다 크면 극단값이 정규분포 가정보다 훨씬 자주 나온다는 뜻이라, 정규분포로 "
        "계산한 VaR 는 실제 손실을 과소평가한다. 일간 주식 수익률은 보통 5~10이다."),
    "표준오차": (
        "표준오차 (Standard Error)",
        "추정값 자체가 얼마나 흔들리는지. 기대수익률의 표준오차는 σ/√T 라 표본이 짧으면 거의 언제나 추정값만큼 "
        "크다. '연 12% 기대'라는 값의 표준오차가 15%면 그 숫자는 0과 구별되지 않는다."),
    "신뢰구간": (
        "신뢰구간 (Confidence Interval)",
        "같은 절차를 반복했을 때 참값을 포함하는 구간. '이 안에 있을 확률 95%'가 아니라 '이 절차가 95%의 "
        "경우 참값을 담는다' 는 뜻이다. 구간이 0을 가로지르면 방향을 말할 수 없다."),
    "분위수": (
        "분위수 (Quantile)",
        "정렬했을 때 특정 비율 지점의 값. VaR 95%는 손실 분포의 5% 분위수다. 평균과 달리 극단값에 "
        "끌려가지 않아 꼬리를 볼 때 쓴다."),
    "팻테일": (
        "팻테일 (Fat Tail)",
        "정규분포보다 극단값이 두꺼운 꼬리. 시장 수익률은 거의 항상 팻테일이라, 정규분포를 가정한 위험 계산은 최악의 "
        "순간에 가장 크게 틀린다. 이 엔진이 GPD 로 꼬리를 따로 적합하는 이유다."),
    "점프": (
        "점프 (Jump)",
        "연속적인 변동으로 설명되지 않는 불연속 가격 변화. 실적·인수·규제처럼 정보가 한 번에 반영될 때 "
        "생긴다. 점프가 잦은 종목은 손절이 설계대로 체결되지 않으므로 사이즈를 줄여야 한다."),
    "베타": (
        "베타 (Beta)",
        "기준 지수가 1% 움직일 때 이 자산이 몇 % 움직이는가. 시간에 따라 변한다 — 기간을 바꾸면 값이 "
        "달라지는 게 정상이고, 지평 간 차이가 크면 구조 변화 신호다. 고정 상수로 다루면 헤지가 어긋난다."),
    "헤지": (
        "헤지 (Hedge)",
        "반대 방향 포지션으로 특정 위험만 상쇄하는 것. 모든 위험이 지워지지 않는다 — 상쇄하고 남는 "
        "잔여위험(베이시스)이 무엇인지 모르면 헤지가 아니라 위험을 다른 위험으로 바꾼 것이다."),
    "스프레드": (
        "호가 스프레드 (Bid-Ask Spread)",
        "매수호가와 매도호가의 차이. 왕복 거래의 최소 비용이다. 넓은 스프레드는 그 자체로 거래 불가 사유가 "
        "된다 — 기대수익이 스프레드보다 작으면 맞아도 잃는다."),
    "유동성": (
        "유동성 (Liquidity)",
        "가격을 크게 밀지 않고 사고팔 수 있는 정도. 호가가 아니라 내 주문이 가격을 얼마나 움직이는가로 봐야 "
        "한다. 유동성이 얇으면 백테스트 수익은 체결 순간 사라진다."),
    "임팩트": (
        "시장 충격 (Market Impact)",
        "내 주문 자체가 가격을 밀어 올리는(내리는) 비용. 주문 크기의 제곱근에 대략 비례한다 — 4배 크게 사면 "
        "비용은 2배가 아니라 참여율까지 고려해야 한다. 전략 수용력(capacity)의 상한을 정하는 요인이다."),
    "듀레이션": (
        "듀레이션 (Duration)",
        "금리가 1%p 움직일 때 채권 가격이 몇 % 변하는가. 만기가 아니라 현금흐름의 가중평균 시점이다. 국채 "
        "ETF 를 주식처럼 베타로만 보면 핵심 위험을 통째로 놓친다."),
    "모멘텀": (
        "모멘텀 (Momentum)",
        "최근 오른 것이 계속 오르는 경향. 가장 오래 살아남은 이상현상이지만 추세 전환에서 급격히 "
        "무너진다(momentum crash). 저변동 구간에서 쌓은 수익을 한 달에 반납하는 패턴이 반복된다."),
    "밸류": (
        "밸류 (Value)",
        "장부가·이익 대비 싼 종목이 초과수익을 낸다는 팩터. 무형자산 비중이 커진 뒤로 장부가 기준 밸류는 설명력이 "
        "크게 떨어졌다 — 팩터가 죽은 것인지 측정이 낡은 것인지를 구분해야 한다."),
    "자기상관": (
        "자기상관 (Autocorrelation)",
        "과거 값이 현재 값을 설명하는 정도. 수익률에 자기상관이 있으면 표본이 겉보기보다 적다 — 독립 관측이 "
        "아니므로 표준오차를 그대로 쓰면 유의성이 부풀려진다. HAC 표준오차가 필요한 이유다."),
    "과적합": (
        "과적합 (Overfitting)",
        "표본 안의 잡음까지 학습해 표본 밖에서 무너지는 상태. in-sample 성적이 좋을수록 의심해야 한다. "
        "여러 설정을 시도한 뒤 최고를 고르는 행위 자체가 과적합이라, 시도 횟수를 보정하지 않은 성과는 "
        "무의미하다."),
    "교차검증": (
        "교차검증 (Cross-Validation)",
        "데이터를 나눠 학습/평가를 번갈아 하는 것. 시계열에서는 그냥 나누면 미래가 과거로 새어 들어간다 — "
        "라벨 구간이 겹치는 표본을 제거하고(purging) 경계에 완충을 두어야(embargo) 결과가 의미를 갖는다."),
    "리밸런싱": (
        "리밸런싱 (Rebalancing)",
        "비중을 목표치로 되돌리는 것. 레버리지 ETP 는 매일 강제 리밸런싱을 하므로 경로에 의존한다 — 지수가 "
        "제자리로 와도 손실이 남는다. 이런 상품에 장기 보유 논리를 적용하면 안 된다."),
}
